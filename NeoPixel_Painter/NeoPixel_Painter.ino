...
#define LED_PIN        6    // пин DIN ленты
#define TRIGGER       A1    // кнопка запуска анимации
#define SPEED         A0    // потенциометр настройки  скорости
#define BRIGHTNESS    A0    // потенциометр настройки яркости
...
void setup() {
  ...
  byte prep[] = {0x73, 0x50, 0x7b, 0x73};
  tm1637.displayByte(prep);
  ...
  if (!sd.begin(CARD_SELECT, SPI_FULL_SPEED)) {
    byte Sder[] = {0x6d, 0x5e, 0x7b, 0x50};
    tm1637.displayByte(Sder);
  ...
  } else {  // если кнопка нажата при старте - используем преобразованные картинки!
  ...
  tm1637.displayByte(0x5e, 0x5c, 0x54, 0x7b);
  delay(500);
}
...
static void error(const __FlashStringHelper *ptr) {
  ...
  tm1637.displayByte(0x79, 0x50, 0x50, _empty);
  for (;;);            // and hang
}
...
void loop() {
  ...
  // ждём нажатия кнопки !!!
  Serial.println("Ready to show");
  while (digitalRead(TRIGGER) == HIGH);

  uint32_t linesPerSec = map(analogRead(SPEED), 0, 1023, 10, maxLPS);  // настройка скорости
  ...
  // ------------ бесконечный цикл
  if (++frame >= nFrames) frame = 0;        // перейти к следующей картинке, если больше числа картинок - обнулить
  Serial.print("Frame #");
